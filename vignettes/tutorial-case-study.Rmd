---
title: "GPMap tutorial: Haemoglobin concentration case study"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{GPMap tutorial: Haemoglobin concentration case study}
...
```{r anchor-trait-metadata, include=FALSE}
gpmapr::select_api(params$api)
anchor_trait_id <- as.integer(params$anchor_trait_id)
anchor_trait_result <- gpmapr::trait(anchor_trait_id)
anchor_trait_name <- anchor_trait_result$trait$trait_name
```

This is a case study of GPMAP genetic colocalisations with the UK Biobank complex trait "`r anchor_trait_name`", a physiological measure underpinned by well understood biological mechanisms of iron uptake, handling and storage and erythropoiesis, which potentially sits downstream of many disease states.
...
**Anchor trait:** `r anchor_trait_name` (id: `r anchor_trait_id`)
...
trait_results <- anchor_trait_result
...
trait_ids_hgb <- dplyr::filter(all_haemoglobin_traits, type_id != anchor_trait_id)
...
This is a recreation of the data you can see on the GPMap website at https://gpmap.opengwas.io/trait.html?id=`r anchor_trait_id`
...
The GPMap found the candidate SNP to be `r tmprss6_variant$variant$snp`, which VEP has annotated as an `r tmprss6_variant$variant$consequence` of `r tmprss6_variant$variant$symbol`.
